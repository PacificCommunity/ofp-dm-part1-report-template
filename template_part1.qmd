...
render_gear_figs_section3 <- function(data, gear_name, gear_code, species_list, species_labels, intro_text = NULL, 
  ...
  # calculate effort (hooks)
  effort_annual <- data_3608 |>
    filter(gear_code == gear_code) |>
    group_by(year, gear_code) |>
    summarise(effort = sum(hooks, na.rm = TRUE))
  ...
  # scaling factor to map effort onto the catch axis
  scale_factor <- max(plot_data |> group_by(year) |> summarise(t = sum(catch)) |> pull(t)) /
    max(effort_annual$effort)
  
  p <- ggplot(plot_data, aes(x = year, y = catch, fill = Species)) + 
    geom_col(position = "stack") + 
    geom_line(data = effort_annual, aes(x = year, y = effort * scale_factor),
              inherit.aes = FALSE, colour = "darkorange",
              linewidth = 1.2, linetype = "dashed") +
    geom_point(data = effort_annual, aes(x = year, y = effort * scale_factor),
    ...
    scale_y_continuous(
      labels = comma,
      sec.axis = sec_axis(~ . / scale_factor, name = "Effort (hooks)", labels = comma)
    ) +
    xlab(element_blank()) +
    ylab("Catch (mt)") +
    theme_bw() +
...
`r country_name`'s Flag vessels for the past 5 years had caught a total of `r fmt(total_5yr)` mt of tuna at an average of `r fmt(avg_total)` mt per year. The average breakdown of catches by species are; SKJ at `r fmt(avg_skj)` mt, YFT at `r fmt(avg_yft)` mt, and BET at `r fmt(avg_bet)` mt per year.
...
#| fig-pos: 'H'
#| label: fig-fig1l
#| fig-cap: !expr if(nrow(data_3605_ll) > 0) glue::glue("Historical annual catch estimates (mt) and effort (no. of hooks) for the {country_name} longline fleet, by primary species and year, in the WCPFC Convention Area for {yr_range}") else ""
#| fig-width: 12
#| fig-height: 7
#| fig-align: "center"
...
#| fig-pos: 'H'
#| label: fig-fig1s
#| fig-cap: !expr if(nrow(data_3605_ps) > 0) glue::glue("Historical annual catch estimates (mt) and effort (no. of hooks) for the {country_name} purse seine fleet, by primary species and year, in the WCPFC Convention Area for {yr_range}") else ""
#| fig-width: 12
#| fig-height: 7
#| fig-align: "center"
...
#| fig-pos: 'H'
#| label: fig-fig1pl
#| fig-cap: !expr if(nrow(data_3605_pl) > 0) glue::glue("Historical annual catch estimates (mt) and effort (no. of hooks) for the {country_name} pole line fleet, by primary species and year, in the WCPFC Convention Area for {yr_range}") else ""
#| fig-width: 12
#| fig-height: 7
#| fig-align: "center"
