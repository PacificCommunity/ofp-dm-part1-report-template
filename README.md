...
- `rewrite_files` — if `TRUE`, all T2 and Ikasavea data are re-downloaded from source; if FALSE, existing files in data/ are reused (only missing files are downloaded)

- `reports_list` — controls which report types are generated (*part1*, *addendum*, *artisanal*)

- `aceman` — If `TRUE`, ACE data generated by your country (usually during TDW, available 
                                                            in T2 report `3619`) will be used in the reports. If `FALSE` the report will use data from 
*yearbook* (T2 report `3605`) which includesall data from last 5 years (if available) and 
after species composition (sually available around August of every year).
